...
%         image = rgb2gray(image);
    ...
    %%% save final result
    accuracy = 100*length(find(result(:, 1)==result(:, 2)))/size(result,1)
    time = toc
    save([path, test_phase{test_phase_num}, '_', mae_phase{1}, '.mat'], 'result', 'save_all_image_path_name',...
        'all_original_scores', 'all_normalization_scores', 'time', 'accuracy')
end
